...
class UNetBlock(nn.Module):
	...
	def forward(self, x):
		...
		if self.outermost:
			...
			return torch.cat([x, self.net(x)], dim=1) if self.add_skip_conn else self.net(x)
...
class Discriminator(nn.Module):
	...
	def __init__(self, in_channels: int, out_channels: int, nb_layers=3, norm_type: str = 'instance'):
		...
		conv = Conv(in_channels, out_channels, kernel_size=4, stride=2, padding=1, bias=True)
		...
		conv = Conv(out_channels * out_f, 1, kernel_size=4, stride=1, padding=1, bias=True)
		layers += [conv]
...
class Pix2Pix(pl.LightningModule):
	...
	def __init__(self, d_lr: float = 2e-4, g_lr: float = 2e-4, beta_1: float = 0.5, beta_2: float = 0.999,
		...
		self.gen = init(CXRdecomp6(batch_size,
									use_norm=use_norm,
									activation_func=activation_func,
									input_views=input_views))
		self.dis = init(Discriminator(in_channels=256 + input_views, out_channels=64, norm_type='instance'))
