...
class KownGenerator(nn.Module):

	def __init__(self, input_views):

		"""
								In: B x D x H x W
								out: B x num_views x D x W
		"""
	...
	def forward(self, x):
		...
		out_drr = (out_drr - torch.min(out_drr)) * (1. / (torch.max(out_drr) - torch.min(out_drr)))
...
class CycleGAN(pl.LightningModule):
	...
	def __init__(self, d_lr: float = 2e-4, g_lr: float = 2e-4, beta_1: float = 0.5, beta_2: float = 0.999,
		...
		self.g_B2A = KownGenerator(input_views)

		self.d_A_params = self.d_A.parameters()
		self.d_B_params = self.d_B.parameters()
		self.g_params = itertools.chain([*self.g_A2B.parameters()])
